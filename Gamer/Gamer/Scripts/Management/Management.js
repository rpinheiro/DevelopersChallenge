//script control championship
$(document).ready(function () {
    ManagementLoad();
});

//associate events to page control
function ManagementLoad() {
    var doc = $(document);

    doc.on("click", "#btnGenerateChampionship", GenerateChampionship);
}

//create championship
function GenerateChampionship() {
    LimparStatus();
    var urlGenerateChampionship = $("#GenerateAction").val();
    var championshipId = $("#Championships option:selected").val();
    if (championshipId == 0) {
        ExibirStatus("Escolha um campeonato.", false);
        return;
    }

    var viewModel = { championshipId: championshipId };
    $.ajax({
        type: "POST",
        url: urlGenerateChampionship,
        data: viewModel,
...
function init(dataChamionship) {
    ...
    // Generates the original graph from an arra y of player names
